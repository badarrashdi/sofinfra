          ...
          {REVIEWS.map((r, idx) => (
            ...
            >
              ...
              <div className="mt-8 pt-4 border-t border-slate-100">
                ...
                <p className="text-[11px] text-[#c59b27] font-medium mt-0.5">{r.location}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="p-6 sm:p-8 rounded-2xl bg-white border border-slate-200/90 shadow-xs grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {TRUST_METRICS.map((metric, idx) => (
            <div key={idx} className="flex flex-col items-center justify-center p-2">
              ...
              <span className="text-xs sm:text-sm font-bold text-[#0b2240]">{metric.label}</span>
              <span className="text-[11px] text-slate-400 mt-0.5">{metric.desc}</span>
            </div>
          ))}
        </div>
      </div>
